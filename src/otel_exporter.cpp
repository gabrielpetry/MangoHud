...
OTelExporter::~OTelExporter() { stop(); }
...
bool OTelExporter::parse_listen(const std::string &listen, std::string &host,
                                std::string &port) {
...
void OTelExporter::maybe_start(const overlay_params *params,
                               uint32_t /*vendorID*/) {
  if (!params)
    return;
  if (!params->enabled[OVERLAY_PARAM_ENABLED_otel]) {
  ...
  if (!m_running.load()) {
    ...
    }
    {
      ...
      m_listen_cached = params->otel_listen;
      m_interval_cached =
          params->otel_interval > 0 ? params->otel_interval : 1000;
      m_startup_delay_cached = params->otel_startup_delay;
    }
    start_locked();
  } else {
    // Already running; detect config changes that require rebuild or restart
    if (m_listen_cached != params->otel_listen ||
        m_interval_cached != params->otel_interval) {
      // For simplicity: full restart if listen address changed; interval change
      // only updated atomically
...
void OTelExporter::start_locked() {
  ...
  try {
    ...
    SPDLOG_INFO(
        "OTel metrics exporter started on {} (interval={}ms, delay={}s)",
        m_listen_cached, m_interval_cached, m_startup_delay_cached);
...
void OTelExporter::build_metrics_string() {
  // Snapshot global values; rely on MangoHud update threads to keep globals
  // fresh.
  Sample s;
  s.fps = fps;
  ...
  s.ram_used =
      currentLogData
          .ram_used; // MiB/GiB? In log it is MiB? Keep as MiB -> rename MB.
...

  {
    ...
    // Label struct for metric labels (key/value pairs)
    struct Label {
      std::string key;
      std::string value;
    };

    // Helper to format a vector of labels into a Prometheus label block
    auto format_labels = [&](const std::vector<Label> &labels) {
    ...
    // Common labels added to every metric. To add new static labels, push
    // them here.
    std::vector<Label> common_labels;
    common_labels.push_back(Label{"pid", std::to_string(m_pid)});
    common_labels.push_back(Label{"exec", m_exec_name});
    // only render this if swapchain is already initialized
    if (HUDElements.sw_stats) {
      common_labels.push_back(Label{"gpu_name", HUDElements.sw_stats->gpuName});
      common_labels.push_back(
          Label{"engine_name", HUDElements.sw_stats->engineName});
      common_labels.push_back(
          Label{"engine_version", HUDElements.sw_stats->engineVersion});
    }
    ...
    std::vector<std::pair<std::string, Metric>> metrics = {
        {"mangohud_fps", {"Current frames per second", "gauge", s.fps}},
        {"mangohud_frametime_ms",
         {"Frame time in milliseconds (most recent frame)", "gauge",
          s.frametime}},
        {"mangohud_cpu_load_percent",
         {"Average CPU load percent", "gauge", s.cpu_load}},
        {"mangohud_gpu_load_percent",
         {"Average GPU load percent", "gauge", s.gpu_load}},
        {"mangohud_cpu_temp_celsius",
         {"CPU temperature in Celsius", "gauge", s.cpu_temp}},
        {"mangohud_gpu_temp_celsius",
         {"GPU temperature in Celsius", "gauge", s.gpu_temp}},
        {"mangohud_cpu_power_watts",
         {"CPU package power draw (W)", "gauge", s.cpu_power}},
        {"mangohud_gpu_power_watts",
         {"GPU power draw (W)", "gauge", s.gpu_power}},
        {"mangohud_ram_used_mb", {"System RAM used (MB)", "gauge", s.ram_used}},
        {"mangohud_vram_used_mb", {"GPU VRAM used (MB)", "gauge", s.vram_used}},
    };

    for (std::vector<std::pair<std::string, Metric>>::const_iterator it =
             metrics.begin();
         it != metrics.end(); ++it) {
      ...
      // Start with common labels; callers can extend by copying this vector
      std::vector<Label> labels = common_labels;
      out << name << format_labels(labels) << " " << metric.value << "\n";
    }
